...
CCrackMeDlg::CCrackMeDlg(CWnd *pParent /*=nullptr*/)
    : CDialogEx(IDD_CRACKME_DIALOG, pParent), key(_T("")), msg(_T("")) {
...
void CCrackMeDlg::DoDataExchange(CDataExchange *pDX) {
    ...
    DDX_Text(pDX, IDC_STATIC_INFO, msg);
    DDX_Control(pDX, IDC_EDIT_KEY, mStatic);
}
...
ON_WM_PAINT()
ON_WM_QUERYDRAGICON()
ON_BN_CLICKED(IDC_BUTTON_ACT, &CCrackMeDlg::OnBnClickedButtonAct)
END_MESSAGE_MAP()
...
BOOL CCrackMeDlg::OnInitDialog() {
    ...
    mEditFont.CreatePointFont(120, _T("Arial"));
    mStatic.SetFont(&mEditFont);
...
void CCrackMeDlg::OnBnClickedButtonAct() {
    UpdateData(TRUE);
    if (validKey(key)) {
        msg = _T("已激活");
    } else {
