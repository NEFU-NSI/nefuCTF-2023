package main

import (
	"encoding/base64"
	"golang.org/x/net/context"
	"net/http"
	"time"
)

func (s *Server) generateResponseFlagGoogle(ctx context.Context, w http.ResponseWriter, r *http.Request, req *DNSRequest) {
	// flag{just_do_a_DoH_query_haha}
	respStr := []byte(`{"Status":0,"TC":false,"RD":true,"RA":true,"AD":true,"CD":false,"Question":[{"name":"www.flag.com","type":1}],"Authority":[{"name":"www.flag.com","type":46,"data":"SOA 8 0 86400 20231006050000 20230923040000 11019 www.flag.com "},{"name":"www.flag.com","type":46,"data":"TXT www.flag.com flag{just_do_a_DoH_query_haha}"},{"name":"www.flag.com","type":46,"data":"NSEC 8 0 86400 20231006050000 20230923040000 11019 www.flag.com "}]}`)

	w.Header().Set("Content-Type", "application/json; charset=UTF-8")
	now := time.Now().UTC().Format(http.TimeFormat)
	w.Header().Set("Date", now)
	w.Header().Set("Last-Modified", now)
	w.Header().Set("Vary", "Accept")
	w.Write(respStr)
}

func (s *Server) generateResponseFlagIETF(ctx context.Context, w http.ResponseWriter, r *http.Request, req *DNSRequest) {
	// flag{this_is_a_easy_DoH}
	base64String := "AAABAAABAAEAAAAAA3d3dwRmbGFnA2NvbQAAEAABwAwAEAABAAAAAAAZGGZsYWd7dGhpc19pc19hX2Vhc3lfRG9IfQ=="
	respBytes, _ := base64.StdEncoding.DecodeString(base64String)

	w.Header().Set("Content-Type", "application/dns-message")
	now := time.Now().UTC().Format(http.TimeFormat)
	w.Header().Set("Date", now)
	w.Header().Set("Last-Modified", now)
	w.Header().Set("Vary", "Accept")
	w.Write(respBytes)
}
